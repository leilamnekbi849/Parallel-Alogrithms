...
#define np 8  // no of processors 
#define nfiles 128  // no of files 
#define filesize 128  // no of inputs in each file 
#define cK 16 // this is number of clusters K cK and it has to be a multiple of np 
...
void sum_validation(int clustersPerNode, int node){
  ...
  if(node)
   ...
   if(node)
    MPI_Send(&convergence_sum,1,MPI_INT,0,0,MPI_COMM_WORLD);
  else 
    for(i=1;i<np;i++){
      MPI_Recv(&others_convergence_sum,1,MPI_INT,i,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      convergence_sum = convergence_sum + others_convergence_sum;
    }
