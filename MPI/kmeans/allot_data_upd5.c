...
#define nfiles 256  // no of files 
#define filesize 256  // no of inputs in each file 
#define cK 256 // this is number of clusters K cK and it has to be a multiple of np 
...
#define range 10001 // this is the max int on any file

float precision = 0.0001; // for checking convergence i.e all centers are the same or not from previous iteration 
...
float centers_copy[np+1][cK/np+1]; // copy for checking changes in centers and convergence


int blocksize = nfiles/np; // the number of files each processor is responsible for 

int data[10000001],dataCount=0;
...
void bcast_and_get_means(int clustersPerNode, int node){
  ...
  for(i=0; i<clustersPerNode; i++){
    ...
    for(k=0;k<np;k++){
      if(k!=node){
	int tag = 10*(node+k) + (node-k)*(node-k) + 100*i;
	MPI_Sendrecv(&clusterMean,1,MPI_FLOAT,k,tag,
		    &centers[k][i],1,MPI_FLOAT,k,tag,MPI_COMM_WORLD,&status);
...
void send_recv_points(int clustersPerNode, int node, int outgoing, int initial_points){
  ...
  // if(!node)
  //printf(" incoming(%d) - outgoing(%d) = %d\n",incoming,outgoing,incoming-outgoing);  // need to put a check point here that sum of all the nodes for this value is 0 **** IF SOMETHING FAILS DO THIS
...
// so they add to existing intact values <-> so its wrong to assume initial = external - (internal + intact)
 ...
 // TEST LOOP 
  ...
  int test_outgoing = 0, flag = 1,invalid = -1;
  for(i=0;i<np;i++){
    if(i!=node){
      for(j=0;j<clustersPerNode;j++){
  ...
  if((test_outgoing != external) || (flag==0)) 
    printf("CHECKPOINT2 FAILED:\n2.1 -> outgoingbuffersize(%d) == expectedsize(%d)?  AND \n2.2 -> all values in expected range (1) == %d, INV = %d\n",test_outgoing,external,flag,invalid);
...
void sum_validation(int clustersPerNode, int node){

  //  later on use this to check sum again from cluster points
  int i = 0,j = 0;
  int  sum = 0, convergence_sum = 0;
  int othersSum = 0, others_convergence_sum = 0;
  int mod = 10007;
  ...
  if(node)
    ...
    for(i=1;i<np;i++){
      MPI_Recv(&othersSum,1,MPI_INT,i,7,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      sum = (sum + othersSum)%mod;
    }
 
  printf("sum calculated by node# %d = %d\n",node,sum);
  
   if(node)
    MPI_Send(&convergence_sum,1,MPI_INT,0,5,MPI_COMM_WORLD);
  else 
    for(i=1;i<np;i++){
      MPI_Recv(&others_convergence_sum,1,MPI_INT,i,5,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      convergence_sum = (convergence_sum + others_convergence_sum)%mod;
    }

  printf("sum calculated by node# %d = %d\n",node,convergence_sum);

  sum %= mod;
  convergence_sum %= mod;
  if(!node)
    if(sum!=convergence_sum)
      printf("CHECKPOINT5 Failed: NET_SUM = %d\n CONVERGENCE SUM = %d ... this is modulo 10007\n",sum,convergence_sum); 
    else printf("CONGRATULATIONS !!! LAST CHECKPOINT(5) PASSED! ITS HIGHLY LIKELY THAT NO DATA IS LOST\n");

}


void copy_centers(int clustersPerNode){

    int i,j;
    for(i=0;i<np;i++)
      for(j=0;j<clustersPerNode;j++)
	centers_copy[i][j] = centers[i][j];
   
    

}

int check_stop_condition(int clustersPerNode){

  int i,j;
   for(i=0;i<np;i++)
      for(j=0;j<clustersPerNode;j++)
	if(100*fabs(centers_copy[i][j]*1.0 - centers[i][j]*1.0) > 100*precision)
	  return 0; // cant stop
...
int main (int argv, char ** argc) {
  ...
  while((iter<MAXITER) && (!check_stop_condition(cK/np))){
    ...
    bcast_and_get_means(cK/np,node);
    check_stop_condition(cK/np);
    iter++;
    if((iter%100==0) & (node==0)){
      printf("ITERATION %d\n",iter);
      print_centers();
    }
